...
# Our data is already in matrix form, so we don't need to create dictionaries
# and call DictVectorizer to make our data into workable matrices.
...
print(
    """2. See if there's anything here that might help your project:
    Firt off, using pandas makes it very easy to ingest csv files into 
    dataframes. From there, I can split off my y vector, edit the features, 
    and check for missing values. I might try this version out when I do my
    exploration. This assignment also made me realize (when I talked about 
    it with Prof Foley) that I need to delete my date and state columns 
    (anything that isn't symptoms and covid case numbers).
    Then running these five models plus the best-of-random model gives me a 
    comprehesive overview of the data, including linear models and non-linear
    models. I can get a good comparison of my models to the best-of-random 
    model. It's also interesting to compare coordinate ascent with SGD. 
    The structure of this code is also a very clear and clean way to run 
    various models and compare their outputs, testing out whether a linear
    or non-linear model is the way to go. 
"""
)
